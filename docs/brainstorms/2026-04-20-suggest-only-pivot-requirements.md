...
- [Affects U2][Design] Dormant-section visual treatment and banner placement (per-section vs top-of-page) — design decision, not technical. Options: banner inside each dormant section with same visual weight as live Suggest section; or Suggest promoted to top + dormant sections collapsed / de-emphasized below a divider; or single top-of-page notice with dormant sections hidden entirely. Default implementation should pick one explicitly.
